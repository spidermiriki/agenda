...
function buildTagsByDay(events) {
  const map = {};
  events.forEach((event) => {
    if (!map[event.date]) map[event.date] = new Set();
    if (event.tagId) map[event.date].add(event.tagId);
  });
  return map;
}
...
function MonthView({ year, month, onBack, onSelectDay, onPrevMonth, onNextMonth }) {
  ...
  const tagsByDay = buildTagsByDay(monthEvents);
  ...
  return (
    <div className="page month-page">
      ...
      <div className="row-nav">
        ...
        <button className="btn btn-nav" onClick={onNextMonth}>
          {'Suivant >'}
        </button>
      </div>
